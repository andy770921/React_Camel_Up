function updatePlayersMoneyArray (state, action) {
    if (state.players.length !== 0 && action.camelsRanking.length !== 0) {
        let copiedPlayerArray = [Object.assign({}, state.players[0]), Object.assign({}, state.players[1]),
        ...
        for (let i = 0; i < state.players.length; i++) {
            for (let j = 0; j < state.players[i].cardStock.length; j++) {
                if (state.players[i].cardStock[j].color === action.camelsRanking[0]) {
                    // 賭中第一名
                    copiedPlayerArray[i].money += parseInt(copiedPlayerArray[i].cardStock[j].rewards);
                } else if (state.players[i].cardStock[j].color === action.camelsRanking[1]) {
                    // 賭中第二名
                    copiedPlayerArray[i].money += 1;
                } else {
                    // 賭中第三或四名
                    copiedPlayerArray[i].money -= 1;
                }
...
export const playerReducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case 'COUNT_ROUND_BET':
            newPlayerArray = updatePlayersMoneyArray(state, action);
            return {
                ...state,
                players: newPlayerArray,
                roundInfo: {
                    camelsRanking: action.camelsRanking, playersMoney: [
        ...
        case 'COUNT_ROUND_BET_NOT_UPDATE_PLAYERS':
            newPlayerArray = updatePlayersMoneyArray(state, action);
            return {
                ...state,
                roundInfo: {
